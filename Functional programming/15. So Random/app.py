...
# 1. Define a function that capitalizes the suffixes


def capitalize_suffix(suffix):
    return suffix.capitalize()  # sau suffix[0].upper() + suffix[1:]


# 2. Apply the capitalize_suffix function to each item in the suffixes list using map()
...
# 3. Generate 10 fantasy names using list comprehensions and the create_fantasy_name function
random_names = [create_fantasy_name(
    prefixes, capitalized_suffixes) for i in range(10)]
...
# 4. Filter the random_names list to include only names that contain 'Fire' and then reduce those names into a single string
...
display_name_info()
"""
Docstring for Intermediate python.Functional programming.15. So Random.app
